...
Indexing is CPU-bound and therefore uses processes rather than threads, avoiding Python's Global Interpreter Lock for preprocessing and duplicate detection. The current configuration starts twelve indexer processes, matching the intended processor count of the development machine; \texttt{MAX\_INDEXERS} should be set to the processor count on another machine. A multiprocessing queue connects the crawler threads to these workers, while a small thread forwards progress messages to the terminal.
...
The indexer also removes near-duplicate content in two steps. It first creates a 64-bit SimHash from weighted character trigrams. SQLite uses Hamming distance to select at most ten close hash candidates, with fewer than five differing bits. Only these candidates receive the more expensive Jaccard-distance comparison over their word sets. Content with a Jaccard distance below 0.02 reuses the existing content record; otherwise its SHA-256 identifier, text, and SimHash are stored. This cascade is faster than comparing every new page with all indexed content and detects duplicates that URL normalisation cannot find.

After crawling, missing document embeddings are calculated with the BERT Large Uncased model. Title, description, and content are encoded together and mean-pooled; vectors are stored for reuse. PageRank is computed for ten iterations with a damping factor of 0.85 over links between ready documents. Both operations are offline because the corpus does not change during querying.

Finally, the ModernBERT AI detector from AICodexLab assigns each unique content record an AI-generation probability. The detector selects CUDA, Apple MPS, or CPU depending on availability and stores the result in \texttt{content.ai\_score}. This score is an optional transparency feature in the interface and does not affect retrieval ranking. Its reliability is limited because the pretrained detector may not match the domain of crawled websites.
